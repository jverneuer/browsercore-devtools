...
describe("diffProfiles — missing-side handling", () => {
    ...
    it("reports differing Date values", () => {
        ...
        registerProfile(b);
        const diff = diffProfiles(a.id, b.id);
        const entry = diff.differences.find((d) => d.path === "expires");
        expect(entry).toBeDefined();
        expect(entry?.a).toBeInstanceOf(Date);
        expect(entry?.b).toBeInstanceOf(Date);
    });
});
