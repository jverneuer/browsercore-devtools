/**
 * Benchmark suite for protocol operations (vendored from @browsercore/testing).
 *
...
import { createFileSystemProvider } from "./file-provider.js";
...
// src/bench -> captures/ (vendored alongside this module).
const capturesDir = fsProvider.join(here, "captures");
...
export function benchmarkTlsHandshake(
    ...
    const bytes = fsProvider.readFileSync(fsProvider.join(capturesDir, "chrome-140", "tls", "client_hello.bin"));
 ...
 * `iterations` runs. This exercises the packet-capture comparison core.
...
export function benchmarkHttp2Request(
    ...
    const bytes = fsProvider.readFileSync(fsProvider.join(capturesDir, "chrome-140", "http2", "settings.bin"));
    const iters = Math.max(1, iterations);
